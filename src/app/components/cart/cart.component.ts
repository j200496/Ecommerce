...
export class CartComponent implements OnInit {
  ...
  abrirWhatsApp() {
    ...
    carrito.forEach((p) => {
      mensaje += `${p.cantidad}  ${p.nombre} ${p.descripcion} = ${
        '$'+p.precio * p.cantidad
      } .00\n`;
    });
    const name = 'A nombre de: '+ this.Nombre;
    const res = mensj + '$' + total+'.00';
    const address = 'Direccion: ' + this.Direccion;
    const url = `https://wa.me/18493577214?text=${encodeURIComponent(
      mensaje + res + '\n' + address + '\n' + name
    )}`;
